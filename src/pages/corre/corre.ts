...
import { GoogleMaps, GoogleMap, GoogleMapOptions, Environment, ILatLng } from '@ionic-native/google-maps/';
...
async stopTracking() {
  ...
  if(this.isTracking){
    ...
    for (let i = 0; i < this.pathLocal.length - 1; i++) {
      console.log(this.getDistanceFromLatLonInMeters(this.pathLocal[i].lat, this.pathLocal[i].lng, this.pathLocal[i + 1]
        .lat, this.pathLocal[i + 1].lng));
      distanciaTotal += this.getDistanceFromLatLonInMeters(this.pathLocal[i].lat, this.pathLocal[i].lng, this.pathLocal[
        i + 1].lat, this.pathLocal[i + 1].lng)
    }

    this.backgroundGeolocation.finish();
  
    if(this.velocidadAceptada(seconds, distanciaTotal)){
      ...
      return this.alertaVelocidadExcedida();
...
velocidadAceptada(segundos: number, metros: number): boolean{
...
alertaVelocidadExcedida(){
  const alertFin = this.alertCtrl.create({
    title: 'Lo sentimos',
    subTitle: 'Hemos detectado que la velocidad con la que corriste fué superior a la de nuestros límites establecidos. No se actualizarán tus estadísticas',
    buttons: [{
      text: 'De acuerdo',
      role: 'De acuerdo',
      handler: () => {
