...
export class CorreTab implements OnInit {
  ...
  startTracking() {
    ...
    if (this.perfil.reto_actual) {
      ...
      .then(() => {
        ...
        this.backgroundGeolocation.on(BackgroundGeolocationEvents.location).subscribe((localizacion: BackgroundGeolocationResponse) => {
          console.log("-------------Segundo-------------")
          this.storePosition(localizacion);
  ...
  async stopTracking() {
    ...
    console.log("Se termino background geolocation")
    ...
    if (position.accuracy < 180) {
      ...
      if (this.pathLocal.length > 1) {
        ...
        this.map.addPolyline(polylineOptions).then((polyline) => {
          ...
          console.log("Precision: " + position.accuracy);
        });
      }else{
        console.log("-------------Se agregó el primer punto-------------")
      }
    }
    else {
      console.log("No se pudo guardar. Accuracy: " + position.accuracy);
    }
